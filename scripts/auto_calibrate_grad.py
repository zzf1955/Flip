...
# ============================================================
# CameraParams: wraps 14 parameters as nn.Parameter
# ============================================================

class CameraParams(nn.Module):
    """14 differentiable camera parameters."""

    def __init__(self, init_dict, device='cpu'):
        super().__init__()
    ...
    def get_param_groups(self, base_lr=1.0):
        """Per-group learning rates (scaled by base_lr)."""
        return [
            {"params": [self.dx, self.dy, self.dz], "lr": base_lr * 0.0001, "name": "position"},
            {"params": [self.pitch, self.yaw, self.roll], "lr": base_lr * 0.01, "name": "angles"},
            {"params": [self.fx, self.fy], "lr": base_lr * 0.1, "name": "focal"},
            {"params": [self.cx, self.cy], "lr": base_lr * 0.05, "name": "principal"},
            {"params": [self.k1, self.k2, self.k3, self.k4], "lr": base_lr * 0.001, "name": "distortion"},
        ]

    def to_array(self):
        return np.array([getattr(self, n).detach().cpu().item() for n in PARAM_NAMES])

    def to_dict(self):
        return {n: getattr(self, n).detach().cpu().item() for n in PARAM_NAMES}
...
def differentiable_fisheye_project(verts_world, cam, ref_t, ref_R, device):
    """
    Differentiable fisheye projection: world 3D -> pixel 2D.

    Args:
        verts_world: (N, 3) float64 tensor, constant (no grad)
        cam: CameraParams module
        ref_t: (3,) float64 tensor, constant per-frame torso translation
        ref_R: (3, 3) float64 tensor, constant per-frame torso rotation

    Returns:
        pts2d: (M, 2) pixel coordinates for visible vertices
        visible: (N,) bool mask
    """
    # Step 1: Camera rotation from Euler angles
    ...
    # Step 3: World -> camera
    p_cam = (R_w2c @ verts_world.T).T + t_w2c  # (N, 3)

    # Step 4: Visibility filter (z > 0.01)
    z = p_cam[:, 2]
    visible = z > 0.01
    p_cam_vis = p_cam[visible]  # (M, 3)

    if p_cam_vis.shape[0] == 0:
        return torch.zeros(0, 2, dtype=torch.float64, device=device), visible

    # Step 5: Fisheye equidistant projection (OpenCV model)
    x = p_cam_vis[:, 0]
    y = p_cam_vis[:, 1]
    z_vis = p_cam_vis[:, 2]
    ...
    # Step 6: Intrinsics
    u = cam.fx * x_prime + cam.cx
    v = cam.fy * y_prime + cam.cy
...
def verify_projection(cam, frames_data, device):
    ...
    cam_pos_np = ref_t_np + ref_R_np @ np.array([dx, dy, dz])
    ...
    pts2d_cv, _ = cv2.fisheye.projectPoints(
        verts_vis.reshape(-1, 1, 3).astype(np.float64), rvec, tvec, K, D)
