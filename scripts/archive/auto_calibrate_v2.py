...
def project_and_mask(link_verts, params, ref_t, ref_R, h, w):
    """Project per-link convex hulls to 2D."""
    ...
    K = build_K(p, _MODEL_CFG)
    D = build_D(p, _MODEL_CFG)
    _fisheye = model_is_fisheye(_MODEL_CFG)
    ...
    for verts3d in link_verts.values():
        ...
        pts2d = project_points_cv(pts3d_valid, rvec, tvec, K, D, _fisheye)
...
def save_comparison(img, gt_mask, pred_mask, iou, params, out_path):
    ...
    p = dict(zip(PARAM_NAMES, params))
    params_text = " ".join(f"{k}={v:.1f}" for k, v in p.items())
...
# Parameter names and bounds from active camera model
