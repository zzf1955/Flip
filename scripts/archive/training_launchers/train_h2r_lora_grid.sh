...
wait_for_slot() {
  while [[ "${#pids[@]}" -ge "${#CUDA_DEVICES_ARR[@]}" ]]; do
    ...
    if [[ "$freed_index" -ge 0 ]]; then
      unset 'pids[freed_index]'
      unset 'pid_cuda[freed_index]'
      pids=("${pids[@]}")
      pid_cuda=("${pid_cuda[@]}")
      return
    fi
...
run_one() {
  ...
  local cmd=(
    ...
    --wandb-tags h2r "layout:${layout_tag}" "rank:${rank}" "run:${RUN_TIMESTAMP}" merge_lora_stack grid_search
  )
