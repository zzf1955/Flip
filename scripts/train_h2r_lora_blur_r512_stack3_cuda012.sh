...
BLUR_MERGE_LORA="${BLUR_MERGE_LORA:-training_data/log/Mitty-blur_r2r_1s-2000d_r512_selfattnqselfattnkselfattnvselfattnoffn0ffn2_1000s_0501_005940/ckpt/step-1000.safetensors}"
...
LORA_LAYOUTS=(ffn qkvo_self ffn_qkvo_self)
...
layout_label() {
  case "$1" in
    ...
    ffn_qkvo_self) printf 'ffn+qkvo(self)' ;;
    *) printf '%s' "$1" ;;
  esac
}
...
run_one() {
  ...
  local run_name="h2r_merge_identity_blur512_${layout}_r${LORA_RANK}_${RUN_TIMESTAMP}"
  local layout_cli=()
  ...
  local cmd=(
    ...
    --wandb-tags h2r "layout:${label}" "layout_key:${layout}" "rank:${LORA_RANK}" "run:${RUN_TIMESTAMP}" merge_identity_blur_r512 cuda012_parallel
  )

  printf '\n[%(%F %T)T] CUDA %s start layout=%s rank=%s\n' -1 "$cuda" "$label" "$LORA_RANK"
  printf 'Merge LoRAs:'
