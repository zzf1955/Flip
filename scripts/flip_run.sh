...
case "$subcommand" in
  ...
  nvidia-smi)
    exec nvidia-smi "$@"
    ;;
  mitty_cache|sam2_precompute|r2h_synthesize|syn_error_analysis|wan_vae_idm|wan_pair_idm|humanoid_pair_idm|adaworld_action_encoder|adaworld_action_decoder|train|train_mitty_mixed_h2r|eval_mitty)
    ;;
  *)
    usage >&2
