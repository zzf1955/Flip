...
- **代码中禁止使用使用 “try-cache”，如果代码出现了预期之外的行为就直接 crash，所有代码必须执行预期的行为。**
- 修改代码的时候不需要对旧代码进行适配，更不能出现错误 fallback 到旧行为。代码更新必须是彻底、完全的升级。
...
> 明确的开发需求请走 `/develop` skill，变更后及时更新对应文档。
...
| 文档 | 内容 |
|------|------|
| `doc/step_4_seedance_api.md` | Seedance 2.0 API 用法 + 输入输出限制 |
| `doc/step_4_wan_vace_regen.md` | ComfyUI + Wan 2.1 VACE depth+mask 重绘 |
...
| `doc/step_5_wan22_ti2v5b.md` | Wan 2.2 TI2V-5B Mitty 方案落地骨干 |
...
| `doc/step_5_ffn_lora_merge.md` | FFN LoRA 训练：合并 identity LoRA + 外观重建 |
