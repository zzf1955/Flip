...
### 命名约定：human2robot 与 h2r/r2h

后续文档和新数据产物按以下规则命名：

- `human2robot`：外部原始配对数据集名。当前本地 legacy 路径仍是
  `data/h2r/v1`，但不要再把该数据集本身写成 `H2R` 或 `HumanAndRobot`。
- `h2r`：FLIP 内部数据方向 / 任务名，表示 human → robot。已有
  `data_type=h2r`、`h2r_2s61f30_slide`、`src.pipeline.h2r_*` 等历史入口保留。
- `r2h`：FLIP 内部数据方向，表示 robot → human。
- human2robot 派生的 duration / cache 名使用数据集名做后缀，例如
  `2s61f30_human2robot_v1`。不要再新建 `2s61f30_h2r_v1` 这类把数据集名和方向名
  混在一起的目录。
...
2s sliding-window 的 `data_type=h2r` cache 已同时包含 human / robot 两侧 latent。若需要训练
robot→human，归档的一次性脚本 `scripts/archive/dataset_utils/build_reverse_r2h_cache.py`
可以生成 `r2h/2s61f30_slide` 视图，而不必重新运行 VAE：

```bash
/home/leadtek/miniconda3/envs/flip/bin/python scripts/archive/dataset_utils/build_reverse_r2h_cache.py
```
...
### human2robot 原始配对 R2H 2s30fps 数据

human2robot v1 的本地 legacy 源路径：

```text
data/h2r/v1/video/<task>/episode_<id>/{robot_camera,human_camera}.mp4
data/h2r/v1/data/<task>/episode_<id>.hdf5
```

当前已物化的 robot → human pair/cache 使用新的 duration 名
`2s61f30_human2robot_v1`：

```text
training_data/pair/r2h/2s61f30_human2robot_v1/<task>/
  video/pair_NNNN.mp4          # human_camera target
  control_video/pair_NNNN.mp4  # robot_camera input
  metadata.csv
  manifest.jsonl
  pair_order.jsonl
training_data/cache/vae/r2h/2s61f30_human2robot_v1/<task>/pair_NNNN.pth
training_data/cache/t5/r2h/2s61f30_human2robot_v1/
```

字段语义：

- `dataset_name=human2robot`，`dataset_legacy_root=data/h2r/v1`。
- `data_type=r2h`，`input_role=robot`，`target_role=human`。
- `control_video/` 是 robot input；`video/` 是 human target。
- 2s30fps 使用 61 帧 `0..60`，每个 episode 只取首窗口，不做滑动窗口。

当前 MP4 源统计：210 个 episode 中 `push_box_two_v1/episode_5` 只有 8 帧，无法构造
61 帧样本；正式 pair/cache 为 209 条，22 个 task。默认 runtime split 已校验为
`train=189`、`in_task_eval=20`、`ood=0`。

HDF5 源使用独立 duration `2s61f30_human2robot_hdf5_v1`，避免和 MP4 版混合：

```text
training_data/pair/r2h/2s61f30_human2robot_hdf5_v1/<task>/
training_data/cache/vae/r2h/2s61f30_human2robot_hdf5_v1/<task>/
training_data/cache/t5/r2h/2s61f30_human2robot_hdf5_v1/
```

HDF5 口径只纳入 `data/h2r/v1/data/**/episode_*.hdf5`，不纳入
`cloth/eval/*.hdf5` 这类非 episode 文件。当前 1312 个 episode 中
`grab_pencil_v1/episode_50.hdf5` 只有 2 帧，`push_box_two_v1/episode_5.hdf5`
只有 8 帧，因此 2s61f30 首窗口可用 1310 条。正式 HDF5 pair/cache 已生成：
1310 条、32 个 task、2620 个 MP4 pair 文件、1310 个 VAE `.pth`。默认 runtime
split 已校验为 `train=1179`、`in_task_eval=131`、`ood=0`。

HDF5 源 pair 生成命令：

```bash
scripts/flip_run.sh human2robot_r2h_pair -- \
  --source hdf5 \
  --tasks all \
  --duration-label 2s61f30_human2robot_hdf5_v1 \
  --resize 224x416 \
  --workers 16 \
  --clean
```

HDF5 源 cache 命令仍复用 `mitty_cache`，但必须显式传独立 T5 目录：

```bash
scripts/flip_run.sh mitty_cache --cuda 0 -- \
  --pair-dir training_data/pair/r2h/2s61f30_human2robot_hdf5_v1/grab_cup_v1 \
  --output training_data/cache/vae/r2h/2s61f30_human2robot_hdf5_v1/grab_cup_v1 \
  --t5-cache-dir training_data/cache/t5/r2h/2s61f30_human2robot_hdf5_v1 \
  --batch-size 1 \
  --prefetch-workers 4 \
  --prefetch-batches 2 \
  --save-workers 2 \
...
如果已有旧 split 目录，可用归档迁移脚本先迁移为新布局；新数据应直接写 task layout：

```bash
python scripts/archive/migration/migrate_task_layout.py --data-type h2r --duration 1s --clean
python scripts/archive/migration/migrate_task_layout.py --data-type blur_r2r --duration 1s --clean
python scripts/archive/migration/migrate_task_layout.py --data-type identity_r2r --duration 1s --clean
```
...
> 归档说明：该 baseline 已移到 `src.pipeline.archive.baseline.masquerade_baseline`，
> 不再接入 `scripts/flip_run.sh` 当前入口；以下内容只作为历史复现记录。

`src.pipeline.archive.baseline.masquerade_baseline` 读取现有 `training_data/pair/h2r/1s/<task>/manifest.jsonl`，
...
```bash
python -m src.pipeline.archive.baseline.masquerade_baseline \
...
> 归档说明：Wan VAE / Pair / Humanoid Everyday / AdaWorld IDM 相关入口已移到
> `src.pipeline.archive.idm.*`，不再接入 `scripts/flip_run.sh` 当前入口；
> 当前下游策略学习保留 `src.pipeline.h2r_diffusion_policy`。

`src.pipeline.archive.idm.wan_vae_idm` 训练一个 Video2Action IDM，用冻结的 Wan 2.2 VAE
...
可选的可见 action mask 由 `src.pipeline.archive.idm.action_mask_precompute` 生成，默认写到
...
只生成当前 h2r 三个 canonical G1 任务的 mask 时可逐任务运行，例如：

```bash
/home/leadtek/miniconda3/envs/flip/bin/python -m src.pipeline.archive.idm.action_mask_precompute \
...
```bash
CUDA_VISIBLE_DEVICES=2 python -m src.pipeline.archive.idm.wan_vae_idm train \
  --device cuda:0 \
...
```bash
CUDA_VISIBLE_DEVICES=2 python -m src.pipeline.archive.idm.wan_vae_idm validate \
  --device cuda:0 \
  --task-short Inspire_Collect_Clothes_MainCamOnly \
...
```bash
CUDA_VISIBLE_DEVICES=2 python -m src.pipeline.archive.idm.wan_vae_idm eval \
  --device cuda:0 \
  --task-short Inspire_Collect_Clothes_MainCamOnly \
  --checkpoint tmp/wan_vae_idm_collect_smoke/best_checkpoint.pt \
...
`src.pipeline.archive.idm.wan_pair_idm` 是独立于 `wan_vae_idm` 的两帧 inverse dynamics
...
Humanoid Everyday H1 子集使用独立 `src.pipeline.archive.idm.humanoid_pair_idm` 入口。该路径直接读取 LeRobot 目录：
...
数据 split、训练命令和指标对比见 `doc/archive/h1_idm_methods.md`。
...
### human2robot / H2R Diffusion Policy BC

`src.pipeline.h2r_diffusion_policy` 是 task068 新增的 human2robot 下游
behavior cloning 入口。当前下载的 human2robot v1 数据不是 LeRobot parquet，
本地 legacy 路径仍为 `data/h2r/v1`：
...
该 bounded smoke 使用两个 human2robot task、4 个 train episode、2 个 val episode、32 个
...
`src.pipeline.h2r_diffusion_policy` 默认仍是 `--dataset-kind h2r_hdf5`，会读取
human2robot HDF5；当前要跑 Unitree G1 2s 数据必须显式传
...
state/action `48` 维，不再读取 human2robot：
...
### human2robot SAM3 blur_r2r 外观训练复现

当前 human2robot 三阶段复现只覆盖前两步：
...
2. step2 使用 human2robot robot-camera 视频和 SAM3/SAM3.1 robot-arm mask 生成
...
3. step3 暂不做。它需要 `(human/synthetic human, real robot)` 配对数据；当前
   已确认 human2robot MP4/HDF5 中存在 `human_camera`，但正式 human→robot 方向仍需要
   先按 human2robot 命名规则构造 `data_type=h2r` 的配对 pair/cache，不能直接复用当前
   `r2h` cache。

SAM3 分割本身是显存较重的前置步骤，先按 `doc/pipeline/sam3_h2r_segmentation.md`
...
生成 human2robot SAM3 blur pair 的 smoke / dry-run：
...
`224x416` 是当前 human2robot SAM3 blur_r2r 的默认尺寸：Wan VAE 后 latent grid 为
...
`scripts/archive/eval_analysis/eval_final_step1000_missing.py` 用于补跑已经训练到 1000 step、但还没有
...
```bash
/home/leadtek/miniconda3/envs/flip/bin/python scripts/archive/eval_analysis/eval_final_step1000_missing.py
```
...
CUDA_ID=2 /home/leadtek/miniconda3/envs/flip/bin/python \
  scripts/archive/eval_analysis/eval_final_step1000_missing.py --execute
```
...
/home/leadtek/miniconda3/envs/flip/bin/python \
  scripts/archive/eval_analysis/eval_final_step1000_missing.py \
  --runner flip_run_2 \
  --cuda-list 0,2,3 \
  --poll-interval 30 \
  --execute
...
> 归档说明：该 mixed h2r 训练入口已移到
> `src.pipeline.archive.training_legacy.train_mitty_mixed_h2r`，不再接入
> `scripts/flip_run.sh`；当前三阶段训练使用 `scripts/flip_run.sh train` /
> `src.pipeline.train`。

`src.pipeline.archive.training_legacy.train_mitty_mixed_h2r` 是独立实验入口，用于混合原始 h2r
...
```bash
CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.run --standalone --nproc_per_node=2 -m src.pipeline.archive.training_legacy.train_mitty_mixed_h2r \
