...
> 归档状态：本地 ComfyUI Wan/Cosmos 重绘已移到
> `src.pipeline.archive.comfyui_wan.*`，不再作为当前主线入口维护；当前数据主线优先走
> Seedance / human2robot / SAM2/SAM3 / 三阶段训练。
...
LD_PRELOAD=/home/leadtek/miniconda3/envs/flip/lib/libjpeg.so.8 \
  python -m src.pipeline.archive.comfyui_wan.cosmos_prepare \
...
```bash
LD_PRELOAD=/home/leadtek/miniconda3/envs/flip/lib/libjpeg.so.8 \
  python -m src.pipeline.archive.comfyui_wan.wan_regen \
...
| `doc/pipeline/seedance.md` | Step 4 商业 API 路径（Seedance） |
