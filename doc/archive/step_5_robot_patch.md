...
> 归档状态：`robot_patch` / `hand_patch` 相关入口已移到 `src.pipeline.archive.patch.*`，
> 不再作为当前三阶段训练主线维护；本文保留为历史实验和复现记录。
...
```bash
# 全量生成（默认 blur）
python -m src.pipeline.archive.patch.robot_patch --task all --degrade blur

# noise 模式
python -m src.pipeline.archive.patch.robot_patch --task all --degrade noise --noise-std 60

# mean 模式（Inspire 任务只取 50 segments）
python -m src.pipeline.archive.patch.robot_patch --task inspire --degrade mean --max-segments 50

# 断点续跑
python -m src.pipeline.archive.patch.robot_patch --task all --degrade blur --resume

# 清空重来
python -m src.pipeline.archive.patch.robot_patch --task all --degrade blur --clean
```
...
python -m src.pipeline.archive.patch.robot_patch \
