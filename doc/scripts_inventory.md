...
| `adaworld_action_decoder.py` | AdaWorld latent action decoder；读取 task054 的 `latent_actions.npz`，按 `episode/chunk/rel_frame_t` 回查 H1 `action` 标签，训练小 MLP 完成 `(frame_t, frame_{t+1}) -> z_t -> action_t` 解码；保存 checkpoint、预测表、loss curve 和 per-dim 诊断指标 | `tmp/adaworld_action_encoder_*` 的 latent artifact + `data/humanoid-everyday-h1-chunks0-6-8-200` | `tmp/adaworld_action_decoder_*` 或 `output/adaworld_action_decoder/humanoid_everyday_h1` |
