...
| `humanoid_pair_idm.py` | Humanoid Everyday H1 两帧 RGB inverse dynamics IDM；直接读取 LeRobot `data/chunk-*/*.parquet`、`videos/chunk-*/egocentric/*.mp4` 和 `meta/tasks.jsonl` / `meta/episodes.jsonl`，输入相邻 egocentric RGB 帧并预测完整 26 维 `action[t]`；默认 `--model-arch transformer`，可切回 `small_cnn` 对照；默认训练不再按 task 切分，仍保留 `--task-indexes` / `--tasks` / `--categories` 作为显式过滤和 per-task metrics 审计工具；输出 train/eval loss、checkpoint、`split_manifest.json`、逐样本预测表和 per-task metrics | `data/humanoid-everyday-h1-chunks0-6-8-200` 这类 Humanoid Everyday H1 LeRobot 子集 | `tmp/humanoid_pair_idm_*` 或 `output/humanoid_pair_idm/humanoid_everyday_h1` |
