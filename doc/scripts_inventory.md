...
| `wan_vae_idm.py` | 冻结 Wan VAE 的 MainCamOnly Video2Action IDM；默认 Collect Clothes，可用 `--task-short` / `--task-full` 切到其他任务；默认 small 3D conv + MLP head 输出 12 维 arm `ee_action` + 12 维 `hand_cmd`，训练中写 `eval_loss.csv`、`best_checkpoint.pt`、`best_val_predictions.csv` 和 `loss_curve.png`，支持 `validate` 复算 held-out checkpoint 指标，也可复算单任务 eval gen/gt action consistency；`eval-h2r` 可用三任务 checkpoint 对 Baseline/Ours H2R `full_eval` 生成视频做 action 复算和汇总 | 原始 `data/unitree_G1_WBT/G1_WBT_<task-short>` + `training_data/segment/<task-short>`；H2R 复算读取 `training_data/log/<run>/full_eval` | `tmp/wan_vae_idm_*`、`output/wan_vae_idm/<task-short>/` 或 `output/idm_h2r_action_eval/` |
