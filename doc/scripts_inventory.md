...
| `h2r_diffusion_policy.py` | H2R / HumanAndRobot Diffusion Policy BC；当前 H2R 下载数据不是 LeRobot parquet，而是 `data/<task>/episode_*.hdf5` + `video/<task>/episode_*/robot_camera.mp4`，入口直接读取 HDF5 中的 `cam_data/robot_camera`、`qpos/qvel/end_position/gripper_state` 和 `action`；clean video/state history 作为 condition，future action chunk 加噪后训练 denoising loss，不预测未来视频；输出 dataset summary、`train_log.jsonl`、best/last checkpoint、eval summary 和 sampled action MSE/per-horizon MSE | `data/h2r/v1` | `tmp/h2r_diffusion_policy_t068*` |
