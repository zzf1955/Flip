...
| `wan_vae_idm.py` | 冻结 Wan VAE 的 Collect Clothes 单任务 Video2Action IDM；默认 small 3D conv + MLP head 输出 12 维 arm `ee_action` + 12 维 `hand_cmd`，训练中写 `eval_loss.csv`、`best_checkpoint.pt`、`best_val_predictions.csv` 和 `loss_curve.png`，支持 `validate` 复算 held-out checkpoint 指标，也可复算 eval gen/gt action consistency | 原始 `data/unitree_G1_WBT/G1_WBT_Inspire_Collect_Clothes_MainCamOnly` + `training_data/segment/Inspire_Collect_Clothes_MainCamOnly` | `tmp/wan_vae_idm_*` 或 `output/wan_vae_idm/` |
