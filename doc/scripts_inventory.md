...
| `wan_pair_idm.py` | 两帧 RGB inverse dynamics IDM；从相邻帧 `(s_t, s_{t+1})` 预测 `action[t]`，arm 使用 `action.ee_action`、hand 使用 `action.hand_cmd`；同一训练入口内训练两套独立小 CNN，不共享参数；输出 train/eval loss、checkpoint 和逐样本预测表 | 原始 `data/unitree_G1_WBT/G1_WBT_<task-short>` + `training_data/segment/<task-short>` | `tmp/wan_pair_idm_*` 或 `output/wan_pair_idm/<task-short>/` |
